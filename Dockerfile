...
LABEL \
  maintainer="Frederico Martins" \
  vendor="Move Your Digital" \
  cmd="docker container run --detach --publish 1194:1194/udp --publish 1194:1194 --publish 443:443 --privileged --device=/dev/net/tun moveyourdigital/pritunl" \
...
# repos
  ...
  apt-key adv --keyserver hkp://keys.gnupg.net --receive-keys --recv 0x656408e390cfb1f5 && \
  echo 'deb http://repo.pritunl.com/stable/apt buster main' \
    > /etc/apt/sources.list.d/pritunl.list && \
  apt-key adv --keyserver hkp://keys.gnupg.net --receive-keys --recv 0x7ae645c0cf8e292a && \
  apt-get -qq update && \
...
# mongodb
  ...
  apt-get -qq -y --no-install-recommends install \
    mongodb-org-server \
    > /dev/null 2>&1 && \
  sed -i -e '/systemLog/,/^$/ s/^/##/' /etc/mongod.conf && \
# pritunl
  apt-get -qq -y --no-install-recommends install \
    iptables \
    iputils-ping \
